...
exports.findOne = function(req, res) {
  ...
  Task.findOne(query, function(err, task) {
    if (err) {
      res
        .status(500)
        .send({ message: "Some error occurred while retrieving task." });
    } else {
      res.send(task);
    }
  });
  ...
  Task.updateOne(
    ...
    function(err, task) {
      if (err) {
        res
          .status(500)
          .send({ message: "Some error occurred while updating task." });
      } else {
        res.send(task);
      }
    }
...
exports.delete = function(req, res) {
  ...
  Task.deleteOne({ _id: id }, function(err) {
    if (err) {
      res
        .status(500)
        .send({ message: "Some error occurred while deleting task." });
    } else {
...
exports.findCompletedTasks = function(req, res) {
  // Retrieve and return all completed tasks from the database.
  ...
  Task.find(query, function(err, tasks) {
    if (err) {
      res
        .status(500)
        .send({
          message: "Some error occurred while retrieving completed tasks."
        });
    } else {
      res.send(tasks);
    }
...
exports.findUncompletedTasks = function(req, res) {
  // Retrieve and return all completed tasks from the database.
...
exports.findDeadlineTasks = function(req, res) {
  // Retrieve and return all completed tasks from the database.
  let query = {
    deadline: { $gte: Date() },
    complete: "No",
...
exports.findOverdueTasks = function(req, res) {
  // Retrieve and return all completed tasks from the database.
  let query = {
    deadline: { $lte: Date() },
    complete: "No",
...
exports.createToken = function(req, res) {
  ...
  const CLIENT_ID =
    "1056929103029-2hm2370o3rg26olnisvvk27l7d13ar56.apps.googleusercontent.com";
